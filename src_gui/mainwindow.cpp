...
void MainWindow::on_btnAddFile_clicked()
{
    ...
    {
        QMessageBox::warning(this, tr("Error"), tr("Given file path was invalid. Please change it to a valid file path."));
        return;
    }
...
void MainWindow::on_btnRemoveFile_clicked()
{
    for(QListWidgetItem *item : ui->listPaths->selectedItems())
    {
        ...
        if(g_status)
            if(!sendPathToDriver((wchar_t*)item->text().toStdWString().c_str(), false))
                return;
...
bool MainWindow::sendPathToDriver(wchar_t* path, bool sendAdd)
{
    ...
    wchar_t hidePath[MAX_PATH + 12]; // Added 12 to cover the command at the begining (e.g. `remove.`)
    bool resultBool;
    ...
    // Write to Driver.
    if(sendAdd)
        wcscpy_s(hidePath, ADD_CMD_PREFIX);
    else
        wcscpy_s(hidePath, REMOVE_CMD_PREFIX);
    wcscat_s(hidePath, devicePath);
